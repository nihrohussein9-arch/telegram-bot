...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [[InlineKeyboardButton("📚 عرض المحاضرات", callback_data="show_weeks")]]
    await update.message.reply_text(
        "أهلاً! أنا بوت المحاضرات 📖\n\n"
        "اضغط الزر أدناه لعرض المحاضرات.",
...
async def show_weeks_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.edit_text("اختر الأسبوع:", reply_markup=InlineKeyboardMarkup(keyboard))


async def lectures_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [[InlineKeyboardButton("📁 " + week, callback_data="week_" + week)] for week in WEEKS]
    await update.message.reply_text("اختر الأسبوع:", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def upload_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args or "|" not in " ".join(context.args):
        ...
        await update.message.reply_text(
            ...
            "مثال:\n"
            "/upload الأسبوع الأول | مقدمة في الفيزياء\n\n"
            "الأسابيع المتاحة:\n" + weeks_list + "\n\n"
            "ثم أرسل ملف PDF"
        )
        return
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(show_weeks_menu, pattern="^show_weeks$"))
